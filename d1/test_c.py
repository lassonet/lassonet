...
DATASET = d1dataset.D1Dataset([os.path.join(DATA_PATH, f) for f in test_file_list], **sl_util.lower_keys(cfg.DATASET))
...
def inference():
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        pointclouds_pl, features_pl, labels_pl, cls_pl = MODEL.placeholder_inputs(NUM_POINTS, val_features_data.shape[-1], len(cfg.DATASET['TRAIN_FILES']))
