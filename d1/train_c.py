...
LOG_DIR = os.path.join(FLAGS.log_dir,  os.path.splitext(os.path.basename(FLAGS.config))[0] + '_'+ datetime.now().strftime("%Y%m%d%H%M"))
...
DATASET = d1dataset.D1Dataset([os.path.join(DATA_PATH, f) for f in train_file_list], **sl_util.lower_keys(cfg.DATASET))
...
def train():
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        train_pc_data, train_features_data, train_label_data, train_cls_data, \
        val_pc_data, val_features_data, val_label_data, val_cls_data = DATASET.strong_split(cfg.NUM_GPUS)
        print('train_pc_data.shape:', train_pc_data.shape, 
              ...
              'train_cls_data.shape', train_cls_data.shape,
              ...
              'val_cls_data.shape', val_cls_data.shape)

        pointclouds_pl, features_pl, labels_pl, cls_pl = MODEL.placeholder_inputs(NUM_POINTS, train_features_data.shape[-1], len(cfg.DATASET['TRAIN_FILES']))
        ...
        train_dataset = tf.data.Dataset \
                .from_tensor_slices((pointclouds_pl, features_pl, labels_pl, cls_pl)) \
