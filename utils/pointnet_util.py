...
def input_transformation_net(point_cloud, is_training, bn_decay=None):
    """ Input (XYZ) Transform Net, input is BxNx (3 + C) gray image
        Return:
            Transformation matrix of size 3xK """
    num_point = point_cloud.get_shape()[1].value
    channel_size = point_cloud.get_shape()[2].value

    input = tf.expand_dims(point_cloud, -1)
    net = tf_util.conv2d(input, 64, [1, channel_size],
    ...
    weights = tf.get_variable('weights', [256, 3*3],
                                initializer=tf.constant_initializer(0.0),
                                dtype=tf.float32)
    biases = tf.get_variable('biases', [3*3],
                                initializer=tf.constant_initializer(0.0),
                                dtype=tf.float32)
    biases = tf.add(biases, tf.constant([1,0,0,
                            0,1,0,
...
def sample_and_group(npoint, radius, nsample, xyz, points, knn=False, use_xyz=True):
    ...
    Output:
        ...
        new_features: (batch_size, npoint, nsample, 3+channel) TF tensor
    ...
    if points is not None:
        new_features = group_point(points, idx) # (batch_size, npoint, nsample, channel)
        if use_xyz:
            new_features = tf.concat([grouped_xyz, new_features], axis=-1) # (batch_size, npoint, nample, 3+channel)
    else:
...
def sample_and_group_all(xyz, points, use_xyz=True):
    ...
    '''
    xyz_shape = tf.shape(xyz) # xyz.get_shape()[0].value
    ...
    if points is not None:
        if use_xyz:
            new_points = tf.concat([xyz, points], axis=2) # (batch_size, nsample, C)
        else:
            new_points = points
        new_points = tf.expand_dims(new_points, 1) # (batch_size, 1, nsample, C)
    else:
        new_points = grouped_xyz
    return new_xyz, new_points, idx, grouped_xyz

def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2,  
                       is_training, bn_decay, scope, group_all = False,
...
def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, mlp_list, 
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            # max pooling
            new_points = tf.reduce_max(grouped_points, axis=[2], keepdims=True, name='maxpool')
            new_points = tf.squeeze(new_points, [2]) # (batch_size, npoints, mlp2[-1])
